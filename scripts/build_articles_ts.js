...
const raw = fs.readFileSync('/tmp/fetched_35_posts.json', 'utf8');
...
function getCategory(title, text) {
  ...
  if (t.includes('soko') || t.includes('pamba') || t.includes('biashara') || t.includes('famasi') || t.includes('uchumi') || t.includes('kiuchumi')) return 'biashara';
...
function getSpecificImage(title, text) {
  ...
  if (t.includes('mama') || t.includes('afya') || t.includes('njiti') || t.includes('fefo') || t.includes('wajawazito')) {
    return 'https://images.unsplash.com/photo-1584515979956-d9f6e5d09982?w=1200&q=80';
  }
  if (t.includes('sekondari') || t.includes('shule') || t.includes('madawati') || t.includes('bwalo') || t.includes('elimu') || t.includes('malezi')) {
    return 'https://images.unsplash.com/photo-1509062522246-3755977927d7?w=1200&q=80';
  }
  if (t.includes('cup') || t.includes('mayeye') || t.includes('mpira') || t.includes('soka')) {
  ...
  if (t.includes('polisi') || t.includes('takukuru') || t.includes('ulinzi') || t.includes('mahakama')) {
    return 'https://images.unsplash.com/photo-1589829545856-d10d557cf95f?w=1200&q=80';
  }
  if (t.includes('soko') || t.includes('usafi') || t.includes('pamba') || t.includes('mpunga')) {
  ...
  if (t.includes('tamasha') || t.includes('urithi')) {
    return 'https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=1200&q=80';
  }
...
function extractParagraphs(rawHtml) {
  const parts = rawHtml
    ...
    .map(p => decodeHtml(p))
    .filter(p => p.length > 20 && !p.startsWith('Source') && !p.includes('The post') && !p.includes('appeared first on'));
...
const articles = posts.map((p, index) => {
  ...
  return {
    ...
    viewsCount: 850 + (posts.length - index) * 65,
    sharesCount: 60 + (posts.length - index) * 9,
    originalUrl: p.link,
    sourceType: 'wordpress'
  };
});

const tsCode = `export const ARTICLES: Article[] = ${JSON.stringify(articles, null, 2)};\n`;
fs.writeFileSync('/tmp/articles_code.ts', tsCode);
console.log('Successfully generated articles TS code. Length:', tsCode.length);
